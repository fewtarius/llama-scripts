...
#   1. Kernel parameters via bootloader (persisted across reboots):
#      - systemd-boot (SteamFork 3.8+): edits loader entry
#      - GRUB (SteamFork 3.7): edits /etc/default/grub + steamfork-grub-update
#   2. amd-smi set -G (runtime hint, applied AFTER bootloader writes so the
#      reboot prompt appears last; uses --no-prompt to suppress confirmation)
#
# Usage:
...
# Capability detection: APU_VRAM_GB is set by detect-gpu.sh from firmware/BIOS
# carveout info. Used to decide whether vis_vramlimit should cap VRAM.
...
try_amd_smi() {
    ...
    if "$amd_smi_path" set -G "$GTT_SIZE_GB" --no-prompt 2>&1; then
...
reset_amd_smi() {
    ...
    if "$amd_smi_path" reset --gtt --no-prompt 2>&1; then
...
# Calculate parameters based on actual system capability, not a hardcoded
# tier. The key decision is whether vis_vramlimit should be set:
#
#   - Large BIOS VRAM (>= 32 GB): the firmware already pre-allocated a large
#     carveout (e.g. Strix Halo up to 96 GB). Setting vis_vramlimit would
#     shrink it - skip it, leave BIOS default. GTT stays small.
#   - Small/Zero BIOS VRAM: the firmware allocated nothing or a tiny
#     amount. Cap vis_vramlimit at 6 GB (handheld-class) so the GPU gets a
#     usable dedicated pool, and grow GTT to fill remaining system RAM.
#   - Medium BIOS VRAM (16-31 GB): cap at 16 GB, moderate GTT growth.
#
# This replaces the old HARDWARE_TIER case statement, which was derived from
# LLAMA_HARDWARE_TIER (a value that no longer drives solver behavior after
# the solver unification). The capability-based check on APU_VRAM_GB
# produces the same result on known hardware but is robust to new APUs.
#
# Override VIS_VRAM_LIMIT_MB or GTT_SIZE_GB via env var or first positional arg.
if [[ "$APU_VRAM_GB" -ge 32 ]]; then
    # Large BIOS carveout (Strix Halo, etc.) - don't cap VRAM.
    VIS_VRAM_LIMIT_MB="${VIS_VRAM_LIMIT_MB:--1}"
    GTT_SIZE_GB="${GTT_SIZE_GB:-4}"
elif [[ "$APU_VRAM_GB" -ge 16 ]]; then
    # Medium carveout (16-31 GB) - generous but bounded cap.
    VIS_VRAM_LIMIT_MB="${VIS_VRAM_LIMIT_MB:-16384}"
else
    # Small/zero carveout (handheld-class) - tight cap, grow GTT.
...
log_info "APU VRAM carveout: ${APU_VRAM_GB}GB"
if [[ "$VIS_VRAM_LIMIT_MB" -gt 0 ]]; then
...
# Apply runtime GTT hint via amd-smi AFTER bootloader params are written.
# This way the reboot prompt (if any) appears last, after all persistent
# changes are complete. Pass --no-prompt to suppress the confirmation.
try_amd_smi || true

log_ok "GPU memory configuration complete."
if [[ "$VIS_VRAM_LIMIT_MB" == "-1" ]]; then
    log_warn "Reboot required for kernel parameter changes to take effect."
    log_warn "vis_vramlimit left at BIOS default (large carveout detected)."
else
    log_warn "Reboot required for kernel parameter changes to take effect."
fi